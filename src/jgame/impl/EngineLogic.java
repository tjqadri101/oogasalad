...
public class EngineLogic {
	...
	* publish this method when finished. */
	public void undefineImage(String name) {
		...
		for (int i=bg_images.size()-1; i>=0; i--) {
			...
			if (bg_image!=null && bg_image.imgname.equals(name)) {
				// XXX we need to call setBGImage instead, otherwise the
				// bg_images trailing nulls won't get cleaned up, and
				// bg_images will stay in extended_mode
	...
	public void defineImageFromData(String name, String tilename,
	int collisionid, int imgwidth, int imgheight,
	int[] imgdata, int imgdataofs,int linewidth,
	String img_op,  int top,int left, int width,int height) {
		if (images_exists.containsKey(name)) {
			undefineImage(name);
		}
		JGImage img = imageutil.createImageFromData(imgwidth,imgheight,imgdata,
			imgdataofs,linewidth);
		defineImage(name,tilename, collisionid, img,
			img_op, top,left, width,height);
	}
	...
	* view always fits within playfield.*/
	...
	public void markAddObject(JGObject obj) {
		...
		System.out.println("markAddObject "+obj.colid);
	}
	...
	boolean do_remove_list) {
		...
		for (int i=obj_to_add.size-1; i>=0; i--) {
			...
			if (prefix==null || obj_to_add.keys[i].startsWith(prefix)) {
				if (cidmask==0 || (o.colid&cidmask)!=0) {
					if (o == null) {System.out.println(obj_to_add.keys[i]);}
					if (suspended_obj || !o.is_suspended) {
...
//		}
	...
	public void setBGImage(String bgimg) {
		setBGImage(bgimg,0,true,true, false);
	}

	public void setBGImage(String bgimg, int depth, boolean wrapx,
	boolean wrapy) {
		setBGImage(bgimg,depth,wrapx,wrapy, true);
	}

	public void setBGImage(String bgimg, int depth, boolean wrapx,
	boolean wrapy,boolean extended_mode) {
		if (extended_mode && !bg_extended_mode) {
			invalidateBGTiles();
			bg_extended_mode=true;
		}
		...
		if (bgimg!=null) {
			...
			while (bg_images.size()>1 
			&& bg_images.elementAt(bg_images.size()-1)==null) {
				bg_images.removeElementAt(bg_images.size()-1);
			}
			// if all bg images are cleared, bg_extended_mode is disabled
			// (regardless of the extended_mode parameter)
			if (bg_images.size()==1 && bg_images.elementAt(0)==null) {
				invalidateBGTiles();
				bg_extended_mode=false;
			}
		}
		invalidateBGTiles();
	}
